...
document.addEventListener("DOMContentLoaded", function () {
  // Select all typed containers
  document.querySelectorAll(".typed-container").forEach((container, index) => {
    const typedElement = container.querySelector(".typed-element");
    const stringsElement = container.querySelector(".typed-strings");

    if (typedElement && stringsElement) {
      new Typed(typedElement, {
        stringsElement: stringsElement,
        typeSpeed: 50 + index * 10, // Vary speed for each
        backSpeed: 30 + index * 10,
        loop: true,
        cursorChar: ["|", "/", "-", "\\"][index % 4], // Rotate cursor styles
      });
    }
  });
});
